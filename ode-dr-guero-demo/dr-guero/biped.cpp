...
static void simLoop (int pause){
	...
	//Sleep(1);			// Windows => Sleep(1), the thread will be suspended for 1 millisecond.
	usleep(1000); 		// Mac => 1000 microseconds = 1 millisecond
	
	// The kbhit() function is a DOS-specific function that checks if a key has been pressed without waiting for the key to be released. 
	// This function is not available on macOS, so you will need to use a different method to check for keyboard input.
	/*if(kbhit()){
		a=getchar();		//�L�[�Ǎ�
		command (a);
	}*/

	// The poll() function takes a pointer to an array of pollfd structures as its first argument. 
	// The second argument is the number of elements in the array. 
	// The third argument is the timeout value. 
	// If the timeout value is 0, the poll() function will return immediately, even if there is no input available.
	// The poll() function returns the number of file descriptors that have input available. 
	// If the poll() function returns 1, then a key has been pressed.
	struct pollfd fds;
	fds.fd = STDIN_FILENO;
	fds.events = POLLIN;
...
int main (int argc, char **argv){
	...
	fn.stop = 0;
	fn.path_to_textures = "/Users/shomu/Downloads/ODE-DrGuero/ODE/ode-0.16.4/drawstuff/textures";
