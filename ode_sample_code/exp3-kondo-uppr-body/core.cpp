...
void core::walk(void){
	...
	case 30:
		...
		//###################  UVC(上体垂直制御) ####################
		//###########################################################
		if(fw<fwMax-1)dvo   = frRatA;			//// 前傾前進 ////
		else dvo=0;
		dvi   += frRatI*(fbRad+dvo);			//// 上体角積分 ////
		...
		//// 遊脚側前振り制御 ////
		if(mode==20||mode==25){								//両脚シフト期間
			if( fwct<(landRate*fwctEnd) ){
				dx[jikuasi^1] = fwr1-(fwr0-dx[jikuasi]);
				if(mode==20&&fwct>2){
					if( (jikuasi==0&&asiPress_r<-0.1) || (jikuasi==1&&asiPress_l<-0.1) ) mode=25;
					else fhOfs+=fhRat;
				}
			}
			else{
